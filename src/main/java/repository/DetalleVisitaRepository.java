...
public interface DetalleVisitaRepository extends JpaRepository<DetalleVisita, Long> {
    ...
    @Query(value = "SELECT \n"
            ...
            + "    av.leido AS leido,\n"
            + "  CASE \n"
            + "    WHEN DATE(v.fecha_visita) < DATE(CURRENT_TIMESTAMP AT TIME ZONE 'America/Guatemala') \n"
            + "      THEN 'Fuera de tiempo'\n"
            + "    ELSE 'A tiempo'\n"
            + "  END AS enTiempo,\n"
            + "  CASE \n"
            + "    WHEN DATE(v.fecha_visita) < DATE(CURRENT_TIMESTAMP AT TIME ZONE 'America/Guatemala') THEN\n"
            + "      CONCAT(\n"
            + "        DATE(CURRENT_TIMESTAMP AT TIME ZONE 'America/Guatemala') - DATE(v.fecha_visita), ' días'\n"
            + "      )\n"
            + "    ELSE NULL\n"
